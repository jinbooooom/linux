...
/* seccomp_user_notification.c
   ...
   This program creates a child process, the "target", while the parent
   process acts as the "supervisor".

   The target process uses seccomp(2) to install a BPF filter using the
   SECCOMP_FILTER_FLAG_NEW_LISTENER flag. This flag causes seccomp(2) to
   return a file descriptor that can be used to receive notifications when
   the filter performs a return with the action SECCOMP_RET_USER_NOTIF;
   the BPF filter employed in this example performs such a return when the
   target process calls mkdir(2).

   The target process passes the notification file descriptor returned by
   seccomp(2) to the supervisor process via a UNIX domain socket.
   (Alternatively, the supervisor might inherit the file descriptor via
   fork(2).)
   ...
   (c) the target process remains blocked in the mkdir(2) system call until
       ...
       call--either a success return value, or a -1 error return with a
       value to be assigned to 'errno').

   The supervisor process receives the notification file descriptor that was
   sent by the target process over the UNIX domain socket. It then waits for
   ...
   From user space, the supervisor is able to do some things that an in-kernel
   seccomp BPF filter can't do; in particular, it can inspect the target
   process's memory (via /proc/PID/mem or some other means such as ptrace(2))
   in order to find out the values referred to by pointer arguments (e.g., the
   pathname argument of mkdir(2)). The supervisor then makes a decision
   based on the pathname, creating the specified directory on behalf of the
   target process only if the pathname starts with ("/tmp/").

   The supervisor then performs a SECCOMP_IOCTL_NOTIF_SEND ioctl(2) operation,
   ...
   an error return, including a value that will be returned in 'errno' in the
   target process.
*/
#define _GNU_SOURCE
#include <sys/types.h>
